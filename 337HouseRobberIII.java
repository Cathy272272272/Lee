/*Holy shit......what have I done this morning!!! so stupid...... though my code is very slow, the logic is so simple......
 * spent several hours for stupid mistakes......
 */
 ...
 * }
 */
class 337HouseRobberIII {
    public int rob(TreeNode root) {
        if ( root == null ) return 0;
        if ( root.left == null && root.right == null ) return root.val;
        if ( root.left == null ) return Math.max (rob(root.right), root.val + rob(root.right.left) + rob(root.right.right));       
        if ( root.right == null ) return Math.max(rob(root.left), root.val + rob(root.left.left) + rob(root.left.right));

...
*/
class 377CombinationSumIV {
    public int combinationSum4(int[] nums, int target) {
        int n = nums.length;
        Arrays.sort(nums);
        Integer[] dp = new Integer[target];
        return help(nums, target, dp);
    }
    public int help(int[] nums, int tar, Integer[] dp) {
        if ( dp[tar-1] != null) return dp[tar-1];
        int count = 0;
        for ( int i : nums ) {
            if ( i > tar ) break;
            if ( i == tar ) {count++; break;}
            count += help(nums, tar - i, dp);
        }
        dp[tar-1] = count;
        return count;
    }
